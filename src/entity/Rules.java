	...
	public static void CheckLoseCondition(Player player) {
		...
		public static void Pledge (Player player) {
			...
			public static void PropertyOption (Player player) {
				...
				String option2 = "Pantsætte grund(e)";
				String option3 = "Sælge Huse";

				String option = GUI.getUserSelection("Hvilke af følgende ting vil du foretage dig?",option1,option2,option3);
				if (option.equals(option2)) {
					...
					while(true) {
						...
						if (player == Gameboard.getField(field-1).getOwner()) {
							if (Gameboard.getField(field-1).getOwner() == null) {
								GUI.showMessage("Du ejer ikke dette felt!");
								continue Pledge;
							}
							if (Gameboard.getField(field-1).getOwner() == player) {
